...
class TanahSpider(scrapy.Spider):
    name = "rumah"
    start_urls = ['https://www.rumah123.com/rumah-dijual/']
    currentpage = 1
    def parse(self,response):
        x = 0
        ToRify()
        for rumah in response.css('div.listing.gts-listing'):
            yield {
            'rumah': rumah.css('h2.listing-title a::text').extract_first(),
            'lokasi': rumah.xpath("//*[@class='listing-location']/@title").extract()[x],
            'luasan': rumah.css('div.land-size b::text').extract_first(),
            'harga': rumah.css('div.price-info::text').extract_first(),
            'penjual': rumah.xpath("//*[@class='btn btn-default contact-agent-btn']/@data-agent-name").extract()[x],
            'penjual_id': rumah.xpath("//*[@class='btn btn-default contact-agent-btn']/@data-agent-id").extract()[x],
            'tanggal_tayang': rumah.css('div.posted-date::text').extract_first(),
            'url': rumah.xpath("//*[@class='btn btn-default contact-agent-btn']/@data-ads-url").extract()[x],
            }
            x=x+1
        ...
        next_page= "/rumah-dijual/page-%s" % page
